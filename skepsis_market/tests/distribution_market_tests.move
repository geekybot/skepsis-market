...
module skepsis_market::distribution_market_tests {
    ...
    fun test_buy_sell_shares() {
        ...
        // Trader C buys 5 tokens in Spread 7 (should cost ~1 USDC according to specs)
        next_tx(scenario, trader_c);
        {
            let mut market = sui::test_scenario::take_shared<skepsis_market::distribution_market::Market<TestCoin>>(scenario);
            let mut registry = sui::test_scenario::take_shared<skepsis_market::distribution_market::UserPositionRegistry>(scenario);
            
            // Get initial price quote for buying 5 tokens
            let spread_index = 7; // Spread 7 (70-80 range)
            let shares_amount = 5_000_000; // 5 with 6 decimals precision
            ...
            sui::transfer::public_transfer(excess, trader_c);
            
            // Check user position
            let (has_position, total_invested, _, _, spread_indices, share_amounts) = 
                skepsis_market::distribution_market::get_user_position(&registry, trader_c, sui::object::id(&market));
            
            assert!(has_position, 200); // User should have a position
            
            sui::test_scenario::return_shared(registry);
            sui::test_scenario::return_shared(market);
        };
        
        // Trader A sells 5 tokens (half of their position) to test selling with slippage
        next_tx(scenario, trader_a);
        {
            let mut market = sui::test_scenario::take_shared<skepsis_market::distribution_market::Market<TestCoin>>(scenario);
            let mut registry = sui::test_scenario::take_shared<skepsis_market::distribution_market::UserPositionRegistry>(scenario);
            
            // Sell half the shares (5 tokens)
            let spread_index = 3; // Spread 3
            let sell_amount = 5_000_000; // 5 tokens with 6 decimals
            
            // Get current sell quote for debugging
            let quote = skepsis_market::distribution_market::get_sell_quote_with_premium<TestCoin>(&market, spread_index, sell_amount);
            ...
            // Check remaining shares (should be 5 tokens = 5,000,000)
            assert!(*vector::borrow(&share_amounts, 0) == 5_000_000, 251);
        ...
        next_tx(scenario, trader_b);
        {
            ...
            // // debug::print(&b"Debug: Trader B total invested: ");
            // // debug::print(&total_invested);
            // // debug::print(&b"Debug: Trader B shares in spread ");
            // // debug::print(vector::borrow(&spread_indices, 0));
            // // debug::print(&b": ");
        ...
        next_tx(scenario, trader_c);
        {
            ...
            // // debug::print(&b"Debug: Trader C total invested: ");
            // // debug::print(&total_invested);
            // // debug::print(&b"Debug: Trader C shares in spread ");
            // // debug::print(vector::borrow(&spread_indices, 0));
            // // debug::print(&b": ");
            // // debug::print(vector::borrow(&share_amounts, 0));
        ...
        next_tx(scenario, trader_a);
        {
            ...
            // // debug::print(&b"Debug: Trader A remaining shares: ");
            // // debug::print(vector::borrow(&share_amounts, 0));
        ...
        next_tx(scenario, trader_b);
        {
            ...
            // Check if the purchase worked
            let (has_position, invested, claimed, _, _, _) = 
                skepsis_market::distribution_market::get_user_position(&registry, trader_a, object::id(&market));
            
            //// // debug::print(&b"Trader A position check - Has position: ");
            //// // debug::print(&has_position);
            //// // debug::print(&b"Trader A invested amount: ");
            //// // debug::print(&invested);
            //// // debug::print(&b"Trader A claimed: ");
            //// // debug::print(&claimed);
            
            assert!(has_position, 101);
            assert!(invested > 0, 102);
            assert!(!claimed, 103);
        ...
        // Trader B buys shares in the 20-30 range (spread 2)
        next_tx(scenario, trader_b);
        {
            //// // debug::print(&b"Trader B buying shares in spread 2 (20-30 range)");
            let mut market = sui::test_scenario::take_shared<skepsis_market::distribution_market::Market<TestCoin>>(scenario);
            let mut registry = sui::test_scenario::take_shared<skepsis_market::distribution_market::UserPositionRegistry>(scenario);
            
            let spread_index = 2; // 3rd spread (20-30 range)
            let shares_amount = 150_000_000; // 150 shares
            
            // Get the cost
            let cost = skepsis_market::distribution_market::get_buy_quote_with_premium<TestCoin>(&market, spread_index, shares_amount);
            //// // debug::print(&b"Cost for Trader B purchase: ");
            //// // debug::print(&cost);
            
            // Mint the coins
            let coins = mint<TestCoin>(cost + 1_000_000, ctx(scenario)); // Add 1 token extra for safety
            ...
            // Check liquidity and share info after purchases
            let (total_shares, cumulative_shares_sold) = 
                skepsis_market::distribution_market::get_market_liquidity_info(&market);
            
            //// // debug::print(&b"Total shares (cap): ");
            //// // debug::print(&total_shares);
            //// // debug::print(&b"Cumulative shares sold: ");
            //// // debug::print(&cumulative_shares_sold);
            
            // Verify shares were sold
            assert!(cumulative_shares_sold == shares_amount + 200_000_000, 104); // Trader A + Trader B
            
            sui::test_scenario::return_shared(registry);
            sui::test_scenario::return_shared(market);
        };
        
        // Trader C buys shares in the 70-80 range (spread 7)
        next_tx(scenario, trader_c);
        {
            //// // debug::print(&b"Trader C buying shares in spread 7 (70-80 range)");
            let mut market = sui::test_scenario::take_shared<skepsis_market::distribution_market::Market<TestCoin>>(scenario);
            let mut registry = sui::test_scenario::take_shared<skepsis_market::distribution_market::UserPositionRegistry>(scenario);
            
            let spread_index = 7; // 8th spread (70-80 range)
            let shares_amount = 50_000_000; // 50 shares
            
            // Get the cost for these shares
            let cost = skepsis_market::distribution_market::get_buy_quote_with_premium<TestCoin>(&market, spread_index, shares_amount);
            //// // debug::print(&b"Cost for Trader C purchase: ");
            //// // debug::print(&cost);
            
            // Mint the coins for the purchase
            let coins = mint<TestCoin>(cost + 1_000_000, ctx(scenario)); // Add 1 token extra for safety
            ...
            // Check if the purchase worked
            let (has_position, invested, claimed, _, _, _) = 
                skepsis_market::distribution_market::get_user_position(&registry, trader_c, object::id(&market));
            
            //// // debug::print(&b"Trader C position check - Has position: ");
            //// // debug::print(&has_position);
            //// // debug::print(&b"Trader C invested amount: ");
            //// // debug::print(&invested);
            //// // debug::print(&b"Trader C claimed: ");
            //// // debug::print(&claimed);
            
            assert!(has_position, 201);
            assert!(invested > 0, 202);
            assert!(!claimed, 203);
        ...
        next_tx(scenario, trader_a);
        {
            // debug::print(&b"Trader A selling all remaining tokens");
            let mut market = sui::test_scenario::take_shared<skepsis_market::distribution_market::Market<TestCoin>>(scenario);
            let mut registry = sui::test_scenario::take_shared<skepsis_market::distribution_market::UserPositionRegistry>(scenario);
            
            // Get user position to see how many shares to sell
            let (_, _, _, _, _spread_indices, share_amounts) = 
                skepsis_market::distribution_market::get_user_position(&registry, trader_a, sui::object::id(&market));
            
            // Sell all remaining shares in spread 3
            let spread_index = 3; // Spread 3 (30-40 range)
            let sell_amount = *vector::borrow(&share_amounts, 0); // Get actual amount of shares owned
            
            // debug::print(&b"Debug: Selling shares amount: ");
            // debug::print(&sell_amount);
            
            // Get current sell quote for debugging
            let quote = skepsis_market::distribution_market::get_sell_quote_with_premium<TestCoin>(&market, spread_index, sell_amount);
            // debug::print(&b"Debug: Expected proceeds for selling all tokens: ");
            // debug::print(&quote);
            ...
            // Check user position after selling - may not even have a position anymore
            let (has_position, _, _, _, _, _) = 
                skepsis_market::distribution_market::get_user_position(&registry, trader_a, sui::object::id(&market));
            
            // debug::print(&b"Debug: Trader A still has position after selling all: ");
            // debug::print(&has_position);
            
            // If user still has a position, check that shares are 0
            if (has_position) {
                let (_, _, _, _, _, share_amounts) = 
                    skepsis_market::distribution_market::get_user_position(&registry, trader_a, sui::object::id(&market));
                
                if (vector::length(&share_amounts) > 0) {
                    let remaining = *vector::borrow(&share_amounts, 0);
                    // debug::print(&b"Debug: Trader A remaining shares: ");
                    // debug::print(&remaining);
                    assert!(remaining == 0, 251);
                }
            };
        ...
        // Trader B sells all remaining tokens to realize profits
        next_tx(scenario, trader_b);
        {
            //// // debug::print(&b"Trader B selling all remaining tokens");
            let mut market = sui::test_scenario::take_shared<skepsis_market::distribution_market::Market<TestCoin>>(scenario);
            let mut registry = sui::test_scenario::take_shared<skepsis_market::distribution_market::UserPositionRegistry>(scenario);
            
            // Sell all remaining shares
            let spread_index = 5; // Spread 5 (50-60 range)
            let sell_amount = 20_000_000; // 20 tokens with 6 decimals
            
            // Get current sell quote for debugging
            let quote = skepsis_market::distribution_market::get_sell_quote_with_premium<TestCoin>(&market, spread_index, sell_amount);
            //// // debug::print(&b"Debug: Expected proceeds for selling 20 tokens: ");
            //// // debug::print(&quote);
            ...
            sui::transfer::public_transfer(proceeds, trader_b);
            
            // Check user position after selling
            let (_, _, _, _, _, share_amounts) = 
                skepsis_market::distribution_market::get_user_position(&registry, trader_b, sui::object::id(&market));
            
            //// // debug::print(&b"Debug: Trader B remaining shares: ");
            //// // debug::print(vector::borrow(&share_amounts, 0));
        ...
        // Trader C sells all remaining tokens to realize profits
        next_tx(scenario, trader_c);
        {
            ...
            sui::transfer::public_transfer(proceeds, trader_c);
        ...
        // Check market state after all trades
        next_tx(scenario, owner);
        {
            // // debug::print(&b"Debug: Checking final market state");
            let market = sui::test_scenario::take_shared<skepsis_market::distribution_market::Market<TestCoin>>(scenario);
            
            // Check total shares sold
            let (_, cumulative_shares_sold) = skepsis_market::distribution_market::get_market_liquidity_info(&market);
            // // debug::print(&b"Debug: Total shares sold (net): ");
            // // debug::print(&cumulative_shares_sold);
            
            // Check outstanding shares for each spread
            let (_, _, _, spread3_shares) = skepsis_market::distribution_market::get_spread_info(&market, 3);
            let (_, _, _, spread5_shares) = skepsis_market::distribution_market::get_spread_info(&market, 5);
            let (_, _, _, spread7_shares) = skepsis_market::distribution_market::get_spread_info(&market, 7);
            
            // // debug::print(&b"Debug: Spread 3 shares: ");
            // // debug::print(&spread3_shares);
            // // debug::print(&b"Debug: Spread 5 shares: ");
            // // debug::print(&spread5_shares);
            // // debug::print(&b"Debug: Spread 7 shares: ");
            // // debug::print(&spread7_shares);
            
            // Verify total shares equals sum of individual spread shares
            assert!(cumulative_shares_sold == spread3_shares + spread5_shares + spread7_shares, 300);
            assert!(spread3_shares == 5_000_000, 301); // 5 tokens left in spread 3
            assert!(spread5_shares == 20_000_000, 302); // 20 tokens in spread 5
            assert!(spread7_shares == 5_000_000, 303); // 5 tokens in spread 7
    ...
    fun test_lp_rewards_and_edge_cases() {
        ...
        next_tx(scenario, trader_a);
        {
           ...
           // // debug::print(&shares_amount);
            ...
            let trade_cost = skepsis_market::distribution_market::get_buy_quote_with_premium<TestCoin>(&market, spread_index, shares_amount);
